...
function updateBar(data: any, sub: any) {
  ...
  const dateString = currentDate.toISOString().split("T")[0]; // Extracts YYYY-MM-DD
  // Combine the date and time strings
  const utcDateString = `${dateString}T${data.ts}Z`; // Append 'Z' to indicate UTC
  // Create a Date object
  const dateObject = new Date(utcDateString);
  const lastBarTime = dateObject.getTime() / 1000;

  const coeff = resolution * 60;
  const rounded = Math.floor(lastBarTime / coeff) * coeff;
